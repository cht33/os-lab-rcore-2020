...
pub trait MemoryHandler: Debug + 'static {
    ...
    fn clone_map(&self, src_pt: Arc<Mutex<PageTableImpl>>, dst_pt: Arc<Mutex<PageTableImpl>>, va: usize, attr: &MemoryAttr);
}
...
impl MemoryHandler for ByFrame {
    ...
    fn clone_map(
        ...
        self.map(dst_pt.clone(), va, attr); // copy the frame
        let mut dst_pt = dst_pt.lock();
        let mut src_pt = src_pt.lock();
        let data = src_pt.get_page_slice_mut(va);
        dst_pt.get_page_slice_mut(va).copy_from_slice(data);
    }
}
